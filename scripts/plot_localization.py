...
def plot_heatmap(data: dict, out: Path):
    ...
    fig, axes = plt.subplots(
        1,
        len(towers),
        figsize=(1.8 + 0.55 * len(layers) * len(towers), 1.2 + 0.5 * len(comps)),
        squeeze=False,
    )
...
def plot_depth(data: dict, out: Path):
    ...
    n_trials = data["summary"].get("n_trials_used", "?")
    fig, axes = plt.subplots(
        1, len(towers), figsize=(5.2 * len(towers), 3.4), squeeze=False, sharey=True
    )
    for ax, tower in zip(axes[0], towers, strict=False):
        for comp in comps:
            xs, ys, los, his = [], [], [], []
            for s in sorted(
                (
                    s
                    for s in sites
                    if s["tower"] == tower and s["component"] == comp and s["layer"] is not None
                ),
